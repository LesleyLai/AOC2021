...
fn main() {
    println!("Part 1: {}", find_least_fuel(INPUT.to_vec().as_slice()));
    println!("Part 2: {}", find_least_fuel2(INPUT.to_vec().as_slice()));
}
...
mod tests {
    ...
    const TEST_INPUT: [i32; 10] = [16, 1, 2, 0, 4, 2, 7, 1, 2, 14];

    #[test]
    fn part1_test() {
        assert_eq!(find_move_cost(TEST_INPUT.to_vec().as_slice(), 1), 41);
        assert_eq!(find_move_cost(TEST_INPUT.to_vec().as_slice(), 2), 37);
        assert_eq!(find_move_cost(TEST_INPUT.to_vec().as_slice(), 3), 39);
        assert_eq!(find_move_cost(TEST_INPUT.to_vec().as_slice(), 10), 71);

        assert_eq!(find_least_fuel(TEST_INPUT.to_vec().as_slice()), 37);
        assert_eq!(find_least_fuel(INPUT.to_vec().as_slice()), 328187);
    }

    #[test]
    fn part2_test() {
        assert_eq!(find_move_cost2(TEST_INPUT.to_vec().as_slice(), 5), 168);
        assert_eq!(find_move_cost2(TEST_INPUT.to_vec().as_slice(), 2), 206);

        assert_eq!(find_least_fuel2(TEST_INPUT.to_vec().as_slice()), 168);
        assert_eq!(find_least_fuel2(INPUT.to_vec().as_slice()), 91257582);
    }
}